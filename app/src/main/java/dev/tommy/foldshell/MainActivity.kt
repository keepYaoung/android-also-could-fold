...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        root.addView(mode)
        label("V2: 커버는 움직임 시작 시 한 장을 캡처하고 검은 그라디언트가 오른쪽에서 들어왔다가 펼칠수록 오른쪽 밖으로 이동하며 사라집니다. 잠금 해제된 커버를 캡처해 왼쪽 변은 고정하고 오른쪽 변만 뒤로 줄입니다. 뒤쪽 영역은 검정으로 채웁니다. 잠금 화면은 캡처 없이 그라디언트만 표시합니다. 센서 추정 기반의 연출이며 실제 공간 고정은 아닙니다. 내부는 왼쪽 절반의 검은 그라디언트가 펼칠수록 사라집니다. 끄면 V1 블러로 돌아갑니다.", 14f)
