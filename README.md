...
| **V5 · stretch + gradient blur, experimental** | Blur gradient over a black gradient shade over the snapshot; the snapshot stretches to the right (left edge pinned) up to 10% with rotation instead of receding, and slides back to native size while it dissolves | The left half stretches to the left with the hinge edge pinned |
